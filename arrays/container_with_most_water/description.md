# Container With Most Water (оиск максимального по площади "аквариума" для воды)

### Оглавление

- [Описание](#description)
- [Брутфорс](#bruteforce_1) - не проходит проверку на leetcode из-за таймаута
- [Брутфорс 2](#bruteforce_2)
...
- Уровень #medium
- Входные данные: массив высот "контейнера"
- Выходные данные: максимальное кол-во воды, которое поместится в "аквариум"
- Пояснение: кол-во воды вычисляется как площадь прямоугольника, 
где высота прямоугольника - минимальная из двух выбранных "высот", а длина - расстояние между высотами во входящем массиве
...
- Выходящие: 49
- Как получился результат: максимальная площадь прямоугольника при выборе второй и последний высот в массиве (8, 7).
Тогда высота квадрата = 7, а длина 7 (индексы "высот" 8 и 1, разница индексов - 7)

###### Пример #2

- Входящие данные: [1,1]
- Выходящие: 1
- Как получился результат: с массивом из 2х элементов возможен только один возможный прямоугольник min(a[0],a[1])*(1-0)

#### Каких кейсов не хватает в примерах

- Пустой массив. Ожидаемое выходное значение - 0
(p.s. в условиях задачи есть ограничение, гарантирующее отсутствие таких кейсов: 2 <= n <= 10<sup>5</sup>)
- Массив из 1го элемента. Ожидаемое выходное значение - 0
  (p.s. в условиях задачи есть ограничение, гарантирующее отсутствие таких кейсов: 2 <= n <= 10<sup>5</sup>)
- Массив состоящий из нулевых элементов ([0,0]/[0,m]). Ожидаемое значение: 0
...
- Поиск всех возможных площадей всех возможных прямоугольников 
с сохранением результатов в промежуточный массив 
...
Для того, чтобы вычислить все возможные площади необходимо перебрать все возможные комбинации 2х элементов в массиве.
Это сводится к "циклу в цикле", что можно прировнять к сложности O(n<sup>2</sup>)

Для того чтобы найти максимум в массиве, нужно перебрать весь получившийся массив один раз.
Так как массив squares имеет размер ~n<sup>2</sup>, то сложность также сводится к O(n<sup>2</sup>).

Т.е. суммарная оценка O(n<sup>2</sup>+n<sup>2</sup>) = O(2n<sup>2</sup>).

Так как при оценке алгоритма нам важен только порядок роста функции (т.е. приниматься будет только
главный член формулы, отбросив все константные множители), итоговая оценка - **O(n<sup>2</sup>)**

###### Память

Для хранения всех площадей нам потребовался дополнительный массив squares, чей размер сравним с n<sup>2</sup>.
...
таким образом мы избавляемся от необходимости хранить доп.массив squares размером n<sup>2</sup> и отдельно вычислять 
максимумы в этом массиве.

Итоговая оценка: n<sup>2</sup> по сложности и константная дополнительная память.
...
### <a name="bruteforce_2"></a> Брутфорс #2
...
(так же как и в [улучшенном брутфорсе](#bruteforce_2) начинаем с прямоугольника максимальной длины). Окно мы будем сужать, пока не дойдем до единичного отрезка в длину.

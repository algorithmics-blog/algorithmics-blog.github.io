...
- [Решение без сортировки](#no_sort)
...
## <a name="no_sort"></a>Решение без сортировки

Для решения задачи этим способом не требуется предварительная сортировка входного массива чисел.
Для решения можно немного видоизменить [реализацию через Hash Set](#hash_set).

Необходимо также проитерироваться по всем элементом массива и на каждой итерации запустить отдельный цикл от `i + 1`.
...
Если такая пара нашлась, то мы нашли искомую тройку.
...
[Решение на GO](./go/solution_no_sort.go)

[Решение на TypeScript](./ts/solution_no_sort.ts)
